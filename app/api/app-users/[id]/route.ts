...
export async function GET(
  ...
  try {
    await connectDB();
    const appUser = await AppUser.findById(params.id);
...
export async function PUT(
  ...
  try {
    ...
    const appUser = await AppUser.findByIdAndUpdate(params.id, body, {
      new: true,
      runValidators: true,
    });
...
export async function DELETE(
  ...
  try {
    await connectDB();
    const appUser = await AppUser.findByIdAndDelete(params.id);
